...
export function FilterBar({ filters }:{ filters:FilterValues }) {
  ...
  return (
    <form className="filter-bar" onSubmit={submit}>
      ...
      <label><span>市场</span><select name="market" defaultValue={filters.market}><option value="">全部市场</option><option>US</option><option>JP</option><option>KR</option><option>TW</option><option>HK</option></select></label>
      ...
      <label><span>状态</span><select name="status" defaultValue={filters.status}><option value="">全部状态</option><option value="confirmed">已确认</option><option value="tba">时间待定</option><option value="rescheduled">已改期</option><option value="cancelled">已取消</option></select></label>
